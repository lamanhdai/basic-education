      ...
      $required_field.each(function(){
        ...
        if($(this).hasClass('name')){
          if(inputVal == ''){
            ...
            switch(lang){
              case "en":;
              case "vn": text = "Please input your full name."; break;
              case "jp": text = "氏名を記入してください";
            }
        ...
        if($(this).hasClass('tel')){
          if(!telReg.test(inputVal)){
            ...
            switch(lang){
              case "en":;
              case "vn": text = "Please input your telephone number."; break;
              case "jp": text = "電話番号を記入してください";
            }
            $parentTag.append('<p class="error">'+text+'</p>');
          }
        }
        if($(this).hasClass('email')){
          if(!emailReg.test(inputVal)){
            ...
            switch(lang){
              case "en":;
              case "vn": text = "Please input your valid email address."; break;
              case "jp": text = "正しいメールアドレスを記入してください";
            }
            $parentTag.append('<p class="error">'+text+'</p>');
          }
        }
        if($(this).hasClass('re_email')){
          ...
          if(!emailReg.test(inputVal) || match_email != inputVal){
            ...
            switch(lang){
              case "en":;
              case "vn": text = "The email and confirmation email addresses do not match."; break;
              case "jp": text = "正しい確認用のメールアドレスを記入してください";
            }
            $parentTag.append('<p class="error">'+text+'</p>');
          }
        }
        if($(this).hasClass('message')){
          if(inputVal == '' || inputVal.length < 20){
            ...
            switch(lang){
              case "en":;
              case "vn": text = "Please input your message at least 20 words."; break;
              case "jp": text = "入力するメッセージは20文字以上です。";
            }
            $parentTag.append('<p class="error">'+text+'</p>');
          }
        }
